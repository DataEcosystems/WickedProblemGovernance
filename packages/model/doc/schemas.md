...
| `t0`                    | Episode Start               | date \| datetime |    No    | —                                                 | `min(t_ev)`   | Timestamp of earliest event indicating entry into an approval workflow.                                  |
| `t1`                    | First Durable Authorization | date \| datetime |    No    | —                                                 | `min(t_auth)` | Timestamp of earliest qualifying authorization event for core scope.                                     |
| `t2`                    | First Delivered Value       | date \| datetime |    No    | —                                                 | `min(t_del)`  | Timestamp of earliest analytic output answering a stakeholder question.                                  |
